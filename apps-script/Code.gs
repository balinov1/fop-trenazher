/**
 * Сервер результатів навчальних тренажерів «Реєстрація ФОП» і «Реєстрація ТОВ».
 ...
 *   action=ping — перевірка зв'язку зі сторінки викладача.
 *
 * Обидва тренажери пишуть в один аркуш; який саме — видно у стовпці «Модуль».
 *
...
var HEADERS = ['Час надходження', 'Студент', 'Група', 'Заняття', '№ картки',
               ...
               'Обрана система', 'Кроки з помилками', 'Модуль'];
...
function doGet(e) {
  ...
  try {
    ...
    else if (p.action === 'ping') out = { ok: true, ping: true, sheet: SpreadsheetApp.getActiveSpreadsheet().getName() };
    else out = { ok: true, hello: 'Сервер тренажера працює. Ця сторінка показується тому, що дію не вказано.' };
  } catch (err) {
...
function sheet_() {
  ...
  if (!sh) {
    ...
    // таблиця створена старішою версією скрипта — дописуємо нові стовпці
...
function list_(p) {
  ...
  var rows = vals.map(function (r) {
    return {
      ...
      n: r[1], g: r[2], l: r[3], c: r[4], s: r[5], k: r[6], m: r[7], kv: r[8], gr: r[9],
      e: r[10], mod: r[11] || 'ФОП'
    };
  });
...
function test_() {
  var r = save_({ token: WRITE_TOKEN, n: 'Перевірка зв\'язку', g: 'ОО-21', l: 'тест',
                  c: 1, s: 12, k: 'TEST', m: 1, kv: '47.81', gr: '1', e: '', mod: 'ФОП' });
